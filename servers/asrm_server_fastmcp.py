...
def list_practice_documents() -> str:
    ...
    """
    documents = asyncio.run(parse_practice_documents())
...
def list_ethics_opinions() -> str:
    ...
    """
    opinions = asyncio.run(parse_ethics_opinions())
...
def search_asrm_guidelines(
    ...
    """
    Search ASRM guidelines by keyword. Can filter by category (practice or ethics).
    """
    results = asyncio.run(search_guidelines(query, category))
...
@mcp.tool()
def get_guideline_content(url: str = Field(description="Full URL of the guideline document")) -> str:
    """
    Retrieve the full content of a specific ASRM guideline document by URL.
    """
    content = asyncio.run(get_guideline_content(url))
